...
export class ListRequestComponent implements OnInit, OnChanges, OnDestroy {
  private unsubscribe$ = new Subject<void>();
  formationComponent: FormationComponent | null = null;
  ...
  constructor(private share: EmployeServiceService, private socketService: SocketService,private router: Router,private route: ActivatedRoute ) {}
  ...
  loadRequests(): void {
    this.isLoading = true;
    this.reqobserv = this.share.getMyRequests().subscribe({
  ...
  // Add this method to handle request updates
  ...
  updateRequest(request: any): void {
    ...
    switch (requestType) {
      case 'Formation':
        ...
        break;
      case 'Congé':
        this.router.navigate(['/collaborateur/conge', request._id], {
          queryParams: { mode: 'edit' }
        });
        break;
      case 'Avance':
        this.router.navigate(['/collaborateur/avance', request._id], {
          queryParams: { mode: 'edit' }
        });
        break;
      case 'Document':
          this.router.navigate(['/collaborateur/document', request._id], {
            queryParams: { mode: 'edit' }
          });
          break;
  ...
  deleteRequest(id: string): void {
    if (confirm('Are you sure you want to delete this request?')) {
      this.isLoading = true;
      this.deleteobserv = this.share.deleteRequest(id).subscribe({
        next: () => {
        ...
        error: (error) => {
          this.showError('An error occurred while deleting the request');
          this.isLoading = false;
          console.error('Error deleting request:', error);
        }
      })
  ...
  getRequestMainType(type: string): string {
    if (['attestation', 'fiche_paie', 'attestation_de_stage'].includes(type)) {
      return 'Document';
    } else if (['internal', 'external'].includes(type)) {
      return 'Formation';
    } else if (['annuel', 'maladie', 'sans_solde', 'maternité', 'paternité'].includes(type)) {
      return 'Congé';
    } else if (['pret', 'avance'].includes(type)) {
      return 'Avance';
    }
    return type;
  }
